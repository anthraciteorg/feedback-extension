...
`;var Lt;!function(t){t[t.Up=1]="Up",t[t.Down=-1]="Down"}(Lt||(Lt={}));const Ft={[Lt.Up]:!0,[Lt.Down]:!1};let Dt=class extends Bt{static styles=h`
        > div:focus-visible {
            opacity: 1;
        }
    ...
    `;#t=Lt.Up;get type(){return this.#t}set type(t){this.#t=t}#e=!1;get active(){return this.#e}set active(t){this.#e=t}get title(){return this.active?"Remove vote":this.type===Lt.Up?"Upvote":"Downvote"}render(){return H`
            ${super.render()}
            <div title="${this.title}" class="mes_button interactable fa-regular fa-thumbs-${this.type===Lt.Up?"up":"down"}" data-i18n="[title]${this.title}" tabindex="0">
            </div>
        `}};e([dt({type:Number})],Dt.prototype,"type",null),e([dt({type:Boolean})],Dt.prototype,"active",null),Dt=e([at("ant-feedback-thumb")],Dt);const zt={[Lt.Up]:[{id:"creative",name:"Creative"},{id:"compelling",name:"Compelling"},{id:"well-structured",name:"Well-structured"},{id:"engaging",name:"Engaging"},{id:"vivid",name:"Vivid"},{id:"insightful",name:"Insightful"}],[Lt.Down]:[{id:"slop",name:"Slop"},{id:"incoherent",name:"Incoherent"},{id:"long-winded",name:"Long-winded"},{id:"repetitive",name:"Repetitive"},{id:"uncreative",name:"Uncreative"},{id:"censored",name:"Censored"}]};let Ht=class extends Bt{static styles=h`
        :host {
            #tags {
                display: flex;
                flex-wrap: wrap;
    ...
    `;#t;get type(){return this.#t}set type(t){this.#t=t}#s={};get selected(){return this.#s}set selected(t){this.#s=t}render(){return H`
        ...
        `}reset(){this.stuck=!1}_onClick(){this.stuck=!0}_onExplanationInput(t){this.feedback.explanation=t.target.value}_onTagSelect(t){this.feedback.tags=Object.keys(t.detail.selected)}_onClose(t,e){t.stopPropagation(),this.stuck=!1,this.dispatchEvent(new CustomEvent("close",{detail:{submit:e}}))}};e([dt({type:Boolean})],Vt.prototype,"open",null),e([dt({type:String,reflect:!0})],Vt.prototype,"anchor",null),e([dt({type:Boolean})],Vt.prototype,"stuck",null),e([dt({type:Object})],Vt.prototype,"feedback",null),Vt=e([at("ant-feedback-popup")],Vt);let Jt=class extends ot{#h;get _feedback(){return this.#h}set _feedback(t){this.#h=t}#a=!1;get _popupOpen(){return this.#a}set _popupOpen(t){this.#a=t}#c;get _popup(){return this.#c}set _popup(t){this.#c=t}_toggleTimeoutId;_mouseOver=!1;_eventListener;get messageId(){return parseInt(this.closest(".mes").attributes.getNamedItem("mesid").value)}get message(){return t.chat[this.messageId]}constructor(){super(),this._eventListener=this._onSwipe.bind(this),t.eventSource.on(t.event_types.MESSAGE_SWIPED,this._eventListener)}render(){if(null!=this.message?.swipe_id)return this._feedback||this._loadFeedback(),H`
            <div @mouseover="${this._onMouseEvent}" @mouseleave="${this._onMouseEvent}">
                <ant-feedback-thumb type="${Lt.Up}" ?active="${this._feedback?.rating===Lt.Up}" @click="${this._vote}"></ant-feedback-thumb>
                <ant-feedback-thumb type="${Lt.Down}" ?active="${this._feedback?.rating===Lt.Down}" @click="${this._vote}"></ant-feedback-thumb>
        ...
        `}async _loadFeedback(){this.message.swipe_id&&this.message.swipe_info&&this.message.swipe_id>=this.message.swipe_info?.length||(this._feedback=new Rt(this.messageId),this._feedback?.load())}_onSwipe(e){this.isConnected?e===this.messageId&&(this._reset(),this._loadFeedback()):t.eventSource.removeListener(this._eventListener)}_deleteFeedback(){this._feedback?.delete(),this._reset()}_onMouseEvent(t){t.relatedTarget&&(this._mouseOver="mouseover"===t.type,this._schedulePopup(this._mouseOver))}async _vote(t){const e=parseInt(t.target.attributes.getNamedItem("type").value);this._feedback=new Rt(this.messageId,this._feedback?.rating!==e?e:void 0),void 0===this._feedback.rating?this._popupOpen=!1:this._schedulePopup(!0),this._submitFeedback()}_schedulePopup(t){this._shouldToggle(t)&&(clearTimeout(this._toggleTimeoutId),this._toggleTimeoutId=window.setTimeout((()=>{this._shouldToggle(t)&&(this._popupOpen=t)}),t?500:750))}_shouldToggle(t){return t!==this._popupOpen&&t===this._mouseOver&&!this._popup.stuck&&void 0!==this._feedback?.rating}_onClose(t){this._popupOpen=!1,t.detail.submit&&(this._feedback?.save(),this._submitFeedback())}_reset(){this._popupOpen=!1,this._popup.reset(),this._feedback=void 0}async _submitFeedback(){if(this._feedback)try{const e=await async function(){let e;const s=new Promise((t=>e=t)),i=t.chat.at(-1);let n={completion:Mt(i.mes),model:i.extra.model,generationType:"textgenerationwebui"===t.main_api?jt.Text:jt.Chat};function r(t){""!==t.prompt&&(n.prompt=n.generationType===jt.Text?Mt(t.prompt):JSON.stringify(Mt(t.chat)),e(n))}t.eventSource.on(t.event_types.GENERATE_AFTER_COMBINE_PROMPTS,r),t.eventSource.on(t.event_types.CHAT_COMPLETION_PROMPT_READY,r),t.Generate("swipe",{},!0);try{return await Promise.race([s,new Promise(((t,e)=>setTimeout((()=>e(new Error("Timeout exceeded"))),1e3)))])}finally{t.eventSource.removeListener(t.event_types.GENERATE_AFTER_COMBINE_PROMPTS,r),t.eventSource.removeListener(t.event_types.CHAT_COMPLETION_PROMPT_READY,r)}}();delete e.generationType;const s=await fetch(`${Ot.container.endpoint}/feedback/${Ot.container.userId}/${this._feedback.messageId}`,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:e.prompt,completion:e.completion,model:e.model,label:this._feedback.rating?Ft[this._feedback.rating]:null,explanation:this._feedback.explanation,tags:this._feedback.tags})});if(!s.ok)throw s;if(!this._feedback.rating)return void this._deleteFeedback();this._feedback.save()}catch(t){if(t instanceof Response)switch(t.status){case 429:case 503:toastr.warning("Rate limit has been reached. Please try again later.","Failed to send feedback");break;default:toastr.error("Invalid response when submitting feedback, please check console for further details.","Failed to send feedback")}else toastr.error("Unknown error occurred, please check console for further details.","Failed to send feedback");if(console.error(t),this._feedback.partiallyFilled)return;this._deleteFeedback()}}};e([ut()],Jt.prototype,"_feedback",null),e([ut()],Jt.prototype,"_popupOpen",null),e([
/**
 * @license
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */
function(t,e){return(e,s,i)=>((t,e,s)=>(s.configurable=!0,s.enumerable=!0,s))(0,0,{get(){return(e=>e.renderRoot?.querySelector(t)??null)(this)}})}("ant-feedback-popup")],Jt.prototype,"_popup",null),Jt=e([at("ant-feedback-controls")],Jt);let Wt=class extends Bt{_saveTimeoutId;static styles=h`
        .menu_button {
            width: initial;
        }
    ...
    `;render(){return H`
        ...
        `}createRenderRoot(){return this}};qt=e([at("ant-feedback-settings-container")],qt),document.querySelector("#message_template .mes_buttons").prepend(document.createElement("ant-feedback-controls")),document.getElementById("extensions_settings").append(document.createElement("ant-feedback-settings-container")),console.info("\n █████╗ ███╗   ██╗████████╗██╗  ██╗██████╗  █████╗  ██████╗██╗████████╗███████╗\n██╔══██╗████╗  ██║╚══██╔══╝██║  ██║██╔══██╗██╔══██╗██╔════╝██║╚══██╔══╝██╔════╝\n███████║██╔██╗ ██║   ██║   ███████║██████╔╝███████║██║     ██║   ██║   █████╗  \n██╔══██║██║╚██╗██║   ██║   ██╔══██║██╔══██╗██╔══██║██║     ██║   ██║   ██╔══╝  \n██║  ██║██║ ╚████║   ██║   ██║  ██║██║  ██║██║  ██║╚██████╗██║   ██║   ███████╗\n╚═╝  ╚═╝╚═╝  ╚═══╝   ╚═╝   ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝╚═╝   ╚═╝   ╚══════╝\nFeedback plugin injected\n");
